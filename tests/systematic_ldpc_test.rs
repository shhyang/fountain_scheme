...
/// [`LDPCLTCode::as_systematic`] must flip [`CodeScheme::code_type`] (regression guard).
#[test]
fn test_ldpc_lt_as_systematic_sets_code_type() {
    let k = 12;
    let mut code = LDPCLTCode::new_with_ideal_soliton(k, XorShift64::new(1));
    assert_eq!(code.code_type(), CodeType::Ordinary);
    code.as_systematic();
    assert_eq!(code.code_type(), CodeType::Systematic);
}

/// Full systematic encode/decode with LDPC precode.
///
/// **Ignored:** `Encoder::new_with_operator` runs a systematic pre-solve over `coded_id in 0..k`;
/// with LDPC (and HDPC) precoding, `SystemSolver` often never reaches [`DecodeStatus::Decoded`],
/// and the encoder panics with `systematic encoding failed` ([`fountain_engine::encoder::Encoder`]).
#[test]
fn test_systematic_ldpc_lt_roundtrip() {
    let k = 26;
    let t = 3;
    ...
    let mut config = LDPCLTCode::new_with_ideal_soliton(k, XorShift64::new(0x00C0_FFEE));
