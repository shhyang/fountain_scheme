...
//! LT Code Example
//!
//! This example demonstrates basic LT (Luby Transform) codes using RandomLTCode.
...
fn main() {
    ...
    let start_id = k; // Start after source symbols
    ...
    for loss_rate in loss_scenarios {
        ...
        // Setup decoder
        let mut decoder =
            Decoder::new_with_operator(&lt_code, Box::new(VecDataOperater::new(symbol_size)));
        ...
        for (i, coded_id) in received_ids.iter().enumerate() {
            ...
            match status {
                ...
                DecodeStatus::NotDecoded => {
                    if i % 5 == 0 && i > 0 {
                        // Print progress every 5 symbols
